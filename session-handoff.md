...
最後更新：2026-07-31 深夜（**102/110 passing，v119**；F106／F107／F103／F108／F110 五條做完。
**F104 實作完成但真機驗出前提被推翻，見下方**）
...
| F103 | 停止後可再開始、回 app 視窗一律消失、±15s 同步 | verify_f103 16/16 ＋ 真機 |
| F108 | 休息綁定發起它的動作（Ryan 真機回報） | verify_f108 20/20 ＋ 真機 |
| F110 | 「回 app 記下一組」跳到所屬動作的計時頁 | verify_f110 11/11 ＋ 真機 |

線上仍是 **v106**，工作樹 v118 **未部署**——Ryan 2026-07-31 決定「先把目前的問題處理完再上線」。

## ⚠ F104 卡住了，要 Ryan 決定方向

實作與 E2E 都完成（16/16），原生視窗也做出來了，但**真機驗出正常路徑在最主要的情境下
走不通**：app 在背景按「記下這組」，那組**會記進去**，可是 JS 整條鏈是等到
**app 回前景**才執行的——視窗因此在 3 秒門檻觸發時顯示「沒記到」。

**這推翻了我稍早的量測結論**：我量到行程在背景仍是 oom_adj 200（fg-service）就推論
「WebView 會活著」。**行程活著 ≠ renderer 沒被暫停**，我把兩件事混為一談了。

四個方向（A 保持一份實作／B 原生自己寫／C 原生排隊回前景補送／D 縮小到前景可用）
寫在 feature_list 的 F104 evidence 裡，含各自的代價。我的意見是先試 A 並實測，
不成再談 C，B 不要走（PRD 關鍵決策二否決過）。

**在 Ryan 決定之前不要繼續往下做。** ⑤ 的失敗態是安全的（不會記重複、不會開假的新一輪），
所以現在的版本放著不會壞事。

## 之後：F105（時間型動作）

acceptance 已凍結，三個開放問題全部有答案。**動工的第一步**是把 `logSet` 從
`renderLogger()` 的閉包裡抽成模組層函式，讓浮動視窗那條路與「完成這組」共用同一份
（④ 明訂記錄邏輯只有一份）。那是動到訓練資料寫入路徑的重構，既有的
verify_f70／f71／f83／f84 會涵蓋回歸，但要在精神好的時候做。

之後是 F105（時間型動作，動資料層，最大的一條）與 F109（360×640 溢出 9px，未簽核）。

## 之後：F104（浮動視窗直接記錄）

acceptance 已凍結，**三個開放問題全部有答案了**，可以直接動工。

開放問題 #1 已實測（2026-07-31，證據見 PRD 的「R4 門檻的依據」）：背景 3 分鐘後
app 行程仍在 **oom_adj 200（perceptible ＋ fg-service）**，一般背景 app 是 900+。
F63 的前景服務只要在跑，就把 WebView 一起釘住——那正是 F104 需要 JS 活著的唯一時段。
**3 秒門檻維持**，它處理的是病態情況（行程仍被回收、renderer 崩掉、ANR），不是常態延遲。

**動工的第一步是把 `logSet` 從 renderLogger() 的閉包裡抽成模組層函式**，讓浮動視窗那條路
與「完成這組」共用同一份（④ 明訂記錄邏輯只有一份）。那是動到訓練資料寫入路徑的重構，
既有的 verify_f70／f71／f83／f84 會涵蓋回歸，但要在精神好的時候做。

Ryan 2026-07-31 已拍板的兩個決定（已寫進 PRD 與條文）：
- 成功回饋＝**短單擊震動 ＋ 視窗內文字**（與鬧鈴的重複震動分得開）
- 累度**不沿用**上一組，就地記的組 rpe 留 null，回 app 進 logger 時提示補（不擋操作）

之後是 F105（時間型動作，動資料層，最大的一條）。

## 舊帳（沒動）

1. **翻新 verify_f48–f60 那 13 支**——F86 ⑩、F87 ⑬⑭、F88 ⑨⑩ 全卡這裡
2. **F89**：⑨ 動效觀感、⑧ onMain 條文與 F63 回歸未逐條驗
   （**⑥ 的規格落差已由 F106 解掉**——軌道與鈕現在是真的存在的零件，可照條文字面驗）
3. **F95 ⑥**：長按通知的手勢，真機
4. verify_f78 ③ 有一條既有 fail（`#000` 與陰影的硬編色，HEAD 就有，非本場造成）
5. **F109（未簽核）**：360×640 休息態垂直溢出 9px。已用 git stash 對照確認是
   v112 那批留下的（最可能是 F102 把 stepper gap 6→8px），不是 F108 造成。
   Ryan 的實機 384×727 量到 0px，所以他看不到
