...
最後更新：2026-07-31 晚（**101/110 passing，v117**；F106／F107／F103／F108 四條做完。
**F110 實作完成、E2E 11/11，但還沒上真機，status 仍是 failing**。本場二度因 usage-guard 收工）
...
線上仍是 **v106**，工作樹 v117 **未部署**——Ryan 2026-07-31 決定「先把目前的問題處理完再上線」。

## 下一條：F110 只差真機

**程式碼與 E2E 都做完了（11/11），status 仍是 failing**——原生那半（按鈕拉起 app 之後
送 focus）沒上過真機，而「停在對的那一頁」正是這條的重點，只有真機看得出來。

接手就三步：出 dev APK → 真機驗（app 已開著、人在別的動作或菜單時按「回 app 記下一組」，
要跳到所屬動作的計時頁）→ 改 passing。細節見 feature_list 的 F110 evidence。

⚠ 版號還是 **v117**（F110 的前端改動尚未 bump）。出 APK 前記得 bump 到 v118（sw.js ＋ state.js 兩處）。
