...
最後更新：2026-07-26（F48–F55 八個 feature 收工；F56 待簽核）

## 現況

**55/55 feature passing**，線上 **v56**，已 deploy（mission-control restart lift-log；本機與公開 `/health` 皆 200、
公開 sw.js 已是 v56）。本輪完成：
...
0. **F56 已擬好 acceptance、等 Ryan 簽核**：/body 圖表加區間選擇（沿用「動作表現」的 1M/3M/6M/9M/1Y/2Y/3Y
   ＋自訂，**預設 3M**，**下方紀錄清單跟著篩**）。後端 `/api/body-metrics` 已支援 `start`／`end`，幾乎純前端；
   前端 `api.listBodyMetrics()` 目前沒帶參數，要加。刻意**不做折點抽樣**（體重是連續數列，聚合會抹掉短期波動；
   長區間若實測難讀再回報）。已知副作用：在 3M 檢視下補記半年前的資料，該筆不會出現在當前清單（flash 會顯示日期）。
