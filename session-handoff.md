...
最後更新：2026-07-30（**待 Ryan 簽核：`docs/signoff-2026-07-30.md`**）

## 🔴 開工前先做這件事

**`docs/signoff-2026-07-30.md` 是一份等 Ryan 一次審完的簽核清單**，涵蓋：
D1 體重頁時間窗要不要留（會廢掉 F56–F58，最貴的一題）／D2 動作表現改長條圖／
F65・F66・F90 的 acceptance／F86–F89 四條改版的 acceptance 草案／順序與 8/1 收官日／五件小事。

**Ryan 沒簽核前不要動 code**——專案規則是 acceptance 簽核凍結後才實作。
他審完給了裁決，就照裁決把條目寫進 `feature_list.json`（標 failing）再開工。
我建議的順序是 **F90 → F66 → F65 → F86 → F87 → F88 → F89**（F90 是資料遺失級，優先於改版）。
