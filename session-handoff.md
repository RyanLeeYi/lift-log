...
最後更新：2026-07-30（**F90 驗收 5/6，卡在一個要 Ryan 拍板的規格缺口**）

## 🔴 等 Ryan 裁決：F90 的 ④ 沒辦法只靠前端成立

`docs/signoff-2026-07-30.md` 的簽核已全數落地（D1 選 B 保留時間窗、D2 照建議、其餘照建議）。
順序照簽核排進 `feature_list.json`，「第一個 failing」就是下一條要做的：
**F90 → F66 → F65 → F86 → F87 → F88 → F89**（共 90 條）。

**F90 實作完成、review 五輪修完、驗收 ①②③⑤⑥ pass，④ fail。**
fail 的不是實作漏做，是 ④ 的條文描述了系統裡不存在的東西——**伺服器沒有「這場訓練已結束」的狀態**。
`結束訓練` 只清 client 快取，`GET /api/workouts/{id}` 照樣回 200（欄位只有
`created_at,date,id,note,sets,template_id`）。所以帶著結束前的舊快取重載，app 會把已結束的訓練接下去。

**這不是假想情境**：手機 APK 與網頁版是兩份快取。在手機按結束，網頁那份不知道，重整就會復活。

兩條路，**選哪條由 Ryan 決定，實作者不得自己改凍結的 acceptance**：

| 選項 | 代價 |
|---|---|
| **A. 補伺服器端結束狀態** | `workouts` 加 `ended_at`、開結束 API、`GET` 回傳它、還原時檢查。動到資料模型與 API，**規模夠大，應該是獨立一條 feature**，F90 ④ 改為引用它 |
| **B. 修正 ④ 的條文** | 承認「已結束」在目前的資料模型裡不可判定，④ 只保留 404 分支，另開一條 feature 處理伺服器端結束狀態（回簽核） |

我建議 **B**：F90 要解的是「訓練被回收後遺失」，那部分已經完整可用；
「跨裝置的結束狀態同步」是另一個問題，混在一起會讓這條無限延長。
但兩者都要開新 feature，差別只在 F90 現在能不能改 passing。

⚠ **在裁決前 F90 維持 failing，也還沒出 APK**（專案規則是改 passing 後才出）。

### F90 已完成的部分（commit `be0785a`..`b344c67`，共 6 筆）
