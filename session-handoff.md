...
最後更新：2026-07-31 下午（**97/102 passing，v112**；F98–F102 五條全部做完）

## 這一場（Ryan 提的五條，全部 passing）

| | 內容 | 驗證 |
|---|---|---|
| F99 | 計時頁組列高度改由列高推導 | verify_f99 16/16 |
| F102 | 重量與次數可直接輸入（± 保留） | verify_f102 18/18 |
| F101 | 上次提示卡改可點視窗、拿掉快調列 | verify_f101 16/16 |
| F98 | 動作卡收合／展開、拖曳時全部收起 | verify_f98 30/30（含觸控節） |
| F100 | 浮動視窗停止＝停響並歸位，✕ 才消失 | 真機逐條 |

線上仍是 **v106**，工作樹 v112 **未部署**——要不要上正式站由你決定（`pwsh scripts/deploy.ps1`）。

## 剩下的（都是舊帳）

1. **翻新 verify_f48–f60 那 13 支**——F86 ⑩、F87 ⑬⑭、F88 ⑨⑩ 三條都卡在這裡，全部不能 passing。
   共用 helper 在 `verify_f67`，照 `tests/e2e/README.md` 改。這是目前最大的一塊
2. **F89**：⑨ 的動效觀感（靜態截圖看不出 160ms 過場）＋ ⑥ 的規格落差要簽核
   （條文寫 switch 的軌道與鈕，實作是藥丸按鈕）
3. **F95 ⑥**：長按通知的手勢，真機

## 這一場學到的（下次直接用）

1. **碰觸控手勢的東西，用滑鼠測一定假綠**。F97 滑鼠 31/31 全綠、真手指完全拖不動——
   捲動容器上的觸控由 compositor 接管，一判定是捲動就送 pointercancel 把拖曳抽走；
   `pointermove` 的 preventDefault 對觸控無效，要擋只能擋 non-passive 的 `touchmove`。
   **verify_f97 與 verify_f98 都已加觸控節**（CDP `Input.dispatchTouchEvent`），新的手勢功能照抄
2. **工具測不出來時，先假設是自己的東西壞了**。我一度把 adb 拖不動歸咎於「注入限制」，
   那是錯的判斷，實際是實作有 bug
3. **原生層的兩態視窗會重建 view**——任何「按下當時設定一次」的狀態，重建後都會被打回預設。
   F100 的暫停鈕就是這樣跑回來的；狀態要在 build 時重新套用
4. **前端與原生的事件會互相抵銷**。F100 的 halt 送 "stop" 給前端，前端回送 ACTION_STOP，
   剛好把要留下的視窗關掉。跨層的事件命名要能分辨「誰該做什麼」
5. **改 `app/static/` 一定要重出 APK 才測得到**（APK 內是打包的資產）；
   設定頁版號沒跳就是還在吃舊版
6. **`app/static/js` 的註解裡不要出現 UI 字元**（↑↓✕ 那類）：verify_f88 ③ 是純文字掃描

## 現況

- **97/102 passing**。剩 F86／F87／F88（卡 13 支腳本）、F89（動效＋簽核）、F95（⑥ 手勢）
- 線上 **v106**（快照 `83080bf`）；工作樹 **v112**，未部署
- Google Drive：`lift-log-v106.apk`（正式）／`lift-log-dev-v112-F100d.apk`（測試）
- **手機**：正式版 v106、測試版 v112-dev 並存。adb 路徑
