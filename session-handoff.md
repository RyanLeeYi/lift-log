...
最後更新：2026-07-26（F48–F56 九個 feature 收工）

## 現況

**56/56 feature passing**，線上 **v57**，已 deploy（mission-control restart lift-log；本機與公開 `/health` 皆 200、
公開 sw.js 已是 v57）。本輪完成：
...
0. **建議下一步：/body 圖表的 x 軸改成時間軸**（F56 review P3-6，未列 feature）。目前 x 軸是等距索引（F8 既有），
   區間可長達 3 年後，時間空缺看不出來、斜率會誤導（例如中間停記兩個月，線看起來仍是等速變化）。
   要做先寫 acceptance 簽核。reviewer 實測效能無虞：1100 天資料畫 1097 點、SVG 11.9KB、切換 74ms。
