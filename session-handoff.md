...
最後更新：2026-08-01 晚（**107/117 passing，v127**；線上 v124）
...
## 隔離區 13 支的翻新（進行中，4/13）

F86 ⑩、F87 ⑬⑭、F88 ⑨⑩ 都卡在這裡。進度：
...
| verify_f50 | ALL PASS |
| verify_f51 | ALL PASS |
| verify_f52 | 剩 1 條真紅 → 另立 **F117**（320×420 雙層捲動） |
| verify_f53–f60 | 尚未開始（8 支） |

**共通失效點已掃過一輪**（套到 f48/f50/f51/f52/f53/f60）：

- `.home-start` → `wait_home` / `start_from_home`（F81 重建首頁）
- 首頁 `.version-tag` → `read_version()`（F81 搬進設定畫面）
- 底部導覽的 emoji 選擇器 → `.bottom-nav .nav-item`（F76 改向量圖示）
- 「← 回首頁」→ `.back-btn`（F81）——**templates 畫面例外**，它至今仍是文字按鈕，
  第一次通掃改過頭、已改回
- 結束訓練 → `.end-workout, .picker-foot .btn-danger`（F83 搬進菜單，兩種版面都要包）
- 菜單項目 `.exercise-item` → `.menu-card`；進度文字「1/3」→ `.menu-bar` 分段指示條（F83）
- 挑課表項目 → `.tpl-choice`（F82）；刪除鈕 ✕ → `.tpl-item-del`（F76／F98）

**原則：只改導覽（怎麼走到那個畫面），不改斷言（驗什麼）。**
唯一動過斷言的是 f50 ⑦——門檻從寫死的 200px 改成「不超過畫面一半」，
因為 F82 把挑課表改成卡片後卡本身就變高了，而 ⑦ 要的是「不被拉長成滿高」。

⚠ **寫檔的教訓（本場踩到）**：用 `Path.write_text()` 寫含 surrogate 跳脫的字串會
先清空檔案再拋 UnicodeEncodeError——`session-handoff.md` 因此被截成 0 bytes 還被 commit 出去
（已從 HEAD~1 還原）。中文內容一律寫成真字元，不要用 `\uXXXX` 拼 emoji。
...
5. **F109（未簽核）**：360×640 休息態垂直溢出，**2026-08-01 起是 14px**（原本 9px）。
   9px 是 v112 那批留下的（最可能是 F102 把 stepper gap 6→8px）；F113 為了修
   384×727 上 39px 的觸控違規補了 min-height: 44，那 +5px 讓它變成 14px。
   **刻意的取捨**——F109 條文本身就寫「不得靠壓觸控目標解決」。
