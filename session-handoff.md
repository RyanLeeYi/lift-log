...
**本場因 usage-guard 收工**，F110 剛提出、未動工）
...
## 下一條：F110（Ryan 剛提的，最小且最急）

**「回 app 記下一組」要跳到那輪休息所屬動作的計時頁。** acceptance 已寫進
`feature_list.json`（**未簽核**，開工前先跟 Ryan 確認條文）。收工前查到的兩件事寫在條文裡：
導頁交前端做（原生只送事件）、`pickExercise` 吃的是完整 exercise 物件不是 id。
