...
最後更新：2026-08-09（**127/138 passing**；**F135 已收**。線上仍 v124，最新正式版 APK 仍是 v139-F130——F135 只出了 dev v148，正式版沒出）

## 接手就看這段（8/9 收官時的狀態）

**F135 → passing（`b79039a`），127/138。** 這場只做兩件事：跑完最後一輪重驗、把 status 收掉。
修法本身是 8/8 做的（獎盃帶＋分層交錯），細節見下一節與 `feature_list.json` 的 F135 evidence。

### 這場的重驗結果

第三輪 acceptance-verifier 針對 `a0e72d2`（v148）的四項改動重驗，全 pass。它自驗做得比要求紮實：

- **`maxRows` 上界**：自己掃 n=41..200000，`plotH` 恆為 69px，確認無任何 n 會讓 y 軸反轉
- **④-c 光暈**：用 `getComputedStyle` 讀真實 `box-shadow` spread（4.0px）交叉核對餘裕 4.50px
- **④ 兩個禁止項**：自寫 2D 重疊判定（不引用 `verify_f135.py` 的 `rect_overlap`），臥推 N=50／肩推 N=34 皆 **0/0**
- **真機 v148**：槓鈴臥推「全部」33 筆，上層 3 顆、下層 5 顆交錯，不重疊不壓點

### 下一場的入口

**F136**（折線圖缺鍵盤／螢幕閱讀器入口）。`touches` 與 F135 完全重疊，只能串行。

**動 `exercise-detail.js` 時順手改一行**：`flagLayout` 的 `maxRows` 註解寫「n≥379」，
**正確值是 n≥347**。兩個數字都對，只是同一顆 commit 把 `FLAG_BAND_GAP` 從 6 改成 9，
首次 `plotH<0` 就從 379 前移到 347，而註解留著 review 當下的舊值。
這次**刻意沒改**——那個檔是 SW 快取的 shell 資產，為一行註解升版號等於作廢剛通過的驗收。

### 待 Ryan 決定：兩條 DEFER 還沒開條目

都是 F135 code review 的產物，**都不是那次改動引入的**：

- **寬螢幕不需要帶卻進帶**：`MIN_CHART_W = 252` 是「320px 視窗」的圖寬，實際圖寬響應式
  （`min(vw,480) − 68`，252–412px）。Ryan 手機 411dp → 圖寬 343px，n=34 真實點距 10.1px > icon 9px，
  **其實不需要帶子**。純視覺退化（繪圖區白白被砍、獎盃離開點正上方）。要根治得量 layout 後的真實寬度。
- **`.line-tip` 蓋住獎盃帶**：tip 釘 `top:0`／`z-index:2`，帶子佔的正是 y=0 起算那 20–22px。
  **8/9 真機上親眼確認**：選中一點後右側那顆獎盃被浮動框整個蓋掉。改動前 tip 一樣會蓋到高點的獎盃。

### 這條 feature 的獨立性折扣（已寫進 evidence，不要洗掉）

**review 與三輪驗收全部是 Claude，跨 context 但不跨模型。** Codex 先是額度用盡；
8/9 額度恢復後**兩度執行逾時被中止**（都沒產出報告，兩次都確認工作區未被修改）。
跨模型驗收這條 feature 始終沒有拿到。Codex 連續逾時本身值得進收官的 harness 檢討，
那是工具面的問題，不是 F135 的問題。

### 環境狀態

- dev server 在 8138 跑（8/8 手動起的，`/health` 回 `{"status":"ok","env":"dev"}`）；prod 8137 未動
- 手機 SM_N9750 裝的是 **dev v148**，density 已 `reset` 還原成 560（Ryan 自己的顯示設定）
- **正式版 APK 沒出**——F135 動到 `app/static/`，要出正式版得另外跑 `-Site prod`。線上站也仍是 v124
- `release-dev\lift-log-v148.apk` 已建置，Google Drive 上是 `lift-log-dev-v148-F135.apk`
