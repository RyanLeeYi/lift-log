...
def main():
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            check("⑩ APP_VERSION 與 sw.js CACHE_NAME 同步遞增（兩處一致，≥v54）",
                  ver.startswith("v") and int(ver[1:]) >= 54
            ...
            check("①⑤ 圖表合成單卡＋toggle（體重／體脂），預設『體重』",
                  page.locator(".body-metric-toggle .chip").count() == 2
                  and on_label == "體重" and cards == 2,  # 圖表卡＋紀錄卡
            ...
            check("① 體重頁籤：清單列全部紀錄且顯示 kg、卡片最新值為 kg",
            ...
            check("②④ 體脂頁籤：清單只列有體脂的日子、單位 %，卡片最新值與範圍也是 %",
            ...
            editing = (page.locator(".bm-row.editing .bm-edit-weight").count() == 1
                       and page.locator(".bm-row.editing .bm-edit-fat").count() == 1)
            page.locator('.bm-row.editing button:has-text("取消")').click()
            page.wait_for_timeout(300)
            check("③ 體脂頁籤下編輯仍是整筆操作（編輯列同時有體重與體脂欄）", editing, f"editing={editing}")
            ...
            check("⑧ 高視窗：紀錄清單填滿剩餘空間（高度隨螢幕變化）、「← 回首頁」完整可見",
            ...
            for h in (667, 560):
                page.set_viewport_size({"width": 390, "height": h})
                page.wait_for_timeout(300)
                page.locator('.screen.body button:has-text("← 回首頁")').scroll_into_view_if_needed()
            ...
            check("⑧/⑨ 矮視窗（667／560）：不強行填滿（清單收在下限）、「← 回首頁」最終完整可見（必要時可捲）",
            ...
            check("review P1-1：矮螢幕不破圖（rows 不溢出清單卡、不疊在「← 回首頁」上）",
            ...
            check("review P1-2：門檻校準後 701–732 無死帶（按鈕在畫面內，或頁面確實可捲得到）",
            ...
            check("review P2-2：切體脂再切回體重，清單捲動位置回到原處",
            ...
            page.locator('.bm-row.editing button:has-text("取消")').evaluate(
                "e => e.dispatchEvent(new MouseEvent('click', { bubbles: true }))")
            page.wait_for_timeout(300)
            check("review P2-1：整頁重繪（進編輯態）後清單捲動位置保留",
            ...
            page.locator(".body-metric-toggle .chip", has_text="體脂").click()
            page.wait_for_timeout(350)
            check("review P3-1：切換頁籤時退出行內編輯態（不留看不見的編輯中狀態）",
            ...
            check("⑧ 門檻語意：清單 ≤2 筆時不加 scrollable",
            ...
            check("⑨ 極矮／橫向視窗回退成整頁可捲，「← 回首頁」捲動後完整可見",
                  all(short.values()), f"{short}")
            ...
            for m in api(base, "GET", "/api/body-metrics"):
                if m.get("body_fat_pct") is not None:
                    api(base, "POST", "/api/body-metrics",
                        {"date": m["date"], "weight_kg": m["weight_kg"]})  # 覆蓋成無體脂
            page.reload()
            page.wait_for_selector(".home-start", timeout=8000)
            page.locator('.btn:has-text("⚖️ 體重")').click()
            page.wait_for_selector(".screen.body", timeout=8000)
            ...
            check("④ 完全沒有體脂紀錄時，體脂頁籤顯示「還沒有紀錄」",
                  "還沒有紀錄" in empty_txt, f"text={empty_txt!r}")
