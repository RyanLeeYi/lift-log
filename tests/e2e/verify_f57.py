...
def main():
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            check("⑦ APP_VERSION 與 sw.js CACHE_NAME 同步遞增（兩處一致，≥v58）",
                  ver.startswith("v") and int(ver[1:]) >= 58
            ...
            check("①② x 軸依時間：兩個月空缺的水平間距遠大於相鄰日的間距（等距索引下會相同）",
            ...
            check("①⑤ x 軸 domain＝選取區間：首點不貼左緣、末點不貼右緣；卡片底部顯示區間邊界",
            ...
            check("⑤ 卡片底部起訖顯示區間邊界（非資料首末點）",
                  today.strftime("%Y-%m-%d") in foot_text
                  and (three_m_ago in foot_text or (today - datetime.timedelta(days=90)).strftime("%Y-%m") in foot_text),
                  f"foot={foot_text!r}")
            ...
            check("② 換長區間（自訂 300 天）後同一批資料的水平跨度變窄（x 軸真的依區間縮放）",
            ...
            dts.nth(0).fill(one_day); dts.nth(1).fill(one_day)
            page.locator(".ex-custom-apply").click()
            page.wait_for_timeout(800)
            poly_one = page.evaluate(POLY)
            rows_one = page.locator(".bm-rows .bm-row").count()
            check("③ from=to 的單日區間：不炸圖、點畫在中央、清單 1 筆",
                  poly_one is not None and len(poly_one["xs"]) == 1
                  and abs(poly_one["xs"][0] - 160) < 1 and rows_one == 1,
            ...
            page.locator(".ex-custom-apply").click()
            page.wait_for_timeout(800)
            check("③ 區間內 0 點：顯示「還沒有紀錄」、無 svg、不炸圖",
                  page.locator(".body-empty").count() >= 1
                  and page.evaluate(POLY) is None,
            ...
            check("⑥ 切體脂後仍是時間軸（同樣的空缺間距）、區間不變、單位 %",
                  poly_fat is not None and len(poly_fat["xs"]) == 12
                  and max(round(poly_fat["xs"][i + 1] - poly_fat["xs"][i], 2)
                          for i in range(len(poly_fat["xs"]) - 1)) > 100
                  and range_kept == "3M" and unit_ok,
            ...
            page.wait_for_timeout(400)
            dots = page.evaluate("() => document.querySelectorAll('.body-chart svg circle[r=\"2\"]').length")
            check("review P2-1：12 個點時每點都畫小圓（含末點的大圓另計）",
                  dots == 12, f"small_dots={dots}（期望 12）")
            ...
            check("review P3-5：最新值旁顯示其量測日期（非區間邊界）",
