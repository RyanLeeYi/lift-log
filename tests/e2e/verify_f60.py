...
def main():
    ...
    try:
        ...
        api(base, "POST", "/api/templates", {"name": "排版檢查課表", "exercises": [
        ...
        with sync_playwright() as pw:
            ...
            check("⑦ APP_VERSION 與 sw.js CACHE_NAME 同步遞增（兩處一致，≥v61）",
                  ver.startswith("v") and int(ver[1:]) >= 61
            ...
            want = [f"負重 0kg × 8 × {SETS[0]} 組"] + [f"20kg × 8 × {SETS[i]} 組" for i in (1, 2, 3)]
            check("①⑥ 每列預設收合成一行摘要（自體重顯示「負重」）、摘要＝F47 預設值×各自 default_sets、無 stepper",
            ...
            check("⑤ 4 列收合時在 390×844 與 360×640 全數完整可見、不需捲動、不溢出、不疊住底部按鈕",
                  all(g["fully_visible"] == 4 and g["rowCount"] == 4 and not g["scrollable"]
                      and g["overflow_modal"] <= 1 and g["over_actions"] <= 1
                      and g["modal_overflow_vp"] <= 1 for g in geo.values()),
            ...
            check("② 點標頭展開出 KG／REPS steppers＋組數控制，再點收合；可同時展開多列（列 2 與列 4）",
                  after_one[1]["open"] and after_one[1]["steppers"] == 2
                  and after_one[1]["sets_ctl"] == 1 and after_one[1]["caret"] == "▾"
                  and not after_one[3]["open"]
                  and after_two[1]["open"] and after_two[3]["open"]
                  and not after_close[1]["open"] and after_close[1]["steppers"] == 0
            ...
            check("③ 點勾選框只切勾選態（.off 生效），收合列不展開、展開列不收合",
                  st3[2]["checked"] is False and st3[2]["off"] and not st3[2]["open"]
                  and st4[3]["checked"] is False and st4[3]["off"] and st4[3]["open"],
            ...
            dis_one_on = page.evaluate(
                "() => document.querySelector('.cal-batch-log').hasAttribute('disabled')")
            check("⑥ 全部取消勾選 → 「全部記錄」disabled 反映在 DOM；勾回一列即解除",
            ...
            page.evaluate("() => { const l = document.querySelector('.cal-batch-list'); "
                          "l.scrollTop = Math.round(l.scrollHeight * 0.45); }")
            page.wait_for_timeout(250)
            before_scroll = page.evaluate("() => document.querySelector('.cal-batch-list').scrollTop")
            ...
            after_scroll = page.evaluate("() => document.querySelector('.cal-batch-list').scrollTop")
            check("④ stepper／組數調整 → 摘要就地更新（22.5kg × 9 × 3 組）、該列不收合、"
                  "其他列展開態與清單捲動位置都保留",
                  after[1]["summary"] == f"22.5kg × 9 × {SETS[1] + 1} 組"
                  and after[1]["open"] and [r["open"] for r in after] == before_open
            ...
            page.wait_for_timeout(400)
            back_to_list = (page.locator(".cal-tpl-list").count() == 1
                            and page.locator(".cal-batch-row").count() == 0)
            check("⑥ 「取消」退回課表清單而非直接關閉（F44 退一步範式）",
                  back_to_list, f"tpl_list={page.locator('.cal-tpl-list').count()}")
            ...
            weights_ok = all(
                ...
                and s["reps"] == 8 and s["rpe"] == 6 for s in sets)
            check("⑥ 重開批次為全收合預設；全部記錄只寫勾選的列、組數／重量／reps／rpe 皆依摘要值",
                  all(not r["open"] for r in fresh) and per == want_per and weights_ok
                  and page.locator(".cal-batch-row").count() == 0,
