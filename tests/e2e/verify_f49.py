...
def main():
    ...
    try:
        ...
        api(base, "POST", "/api/templates", {"name": "F49 課表", "exercises": [
        ...
        with sync_playwright() as pw:
            ...
            check("⑨ APP_VERSION 與 sw.js CACHE_NAME 同步遞增（兩處一致，≥v50）",
                  ver.startswith("v") and int(ver[1:]) >= 50
            ...
            start_with_template()
            check("① 有課表 → 搜尋框／chips／清單／自訂動作皆不在畫面上，只有「＋ 臨時加動作」入口鈕",
            ...
            modal_ok = (page.locator('.pick-modal input[type="search"]').count() == 1
            ...
            scrolls = page.eval_on_selector(".pick-modal .pick-list",
                                            "e => e.scrollHeight > e.clientHeight + 2")
            check("③ 視窗含搜尋／部位 chips／清單（固定高度可捲）／＋自訂動作／取消",
                  modal_ok and scrolls, f"parts={modal_ok} list_scrolls={scrolls}")
            ...
            focused = page.evaluate("document.activeElement && document.activeElement.type === 'search'")
            kept = search.input_value()
            check("⑤ 視窗內搜尋就地更新清單：結果變少、輸入內容與焦點都還在（未整頁重繪）",
            ...
            check("⑤ 部位 chip 就地篩選、再點同一顆取消篩選",
                  on_after_first == 1 and filtered >= 1 and unfiltered > filtered
            ...
            back_in_list = (page.locator(".custom-ex-modal").count() == 0
                            and page.locator(".pick-modal").count() == 1
                            and page.locator(".pick-modal .pick-list .exercise-item",
                                             has_text=custom_name).count() == 1)
            check("⑥ 自訂動作視窗疊在上層；建立後回到動作清單且新動作可選",
                  stacked and back_in_list, f"stacked={stacked} back={back_in_list}")
            ...
            page.locator(".add-exercise-open").click()
            page.wait_for_selector(".pick-modal", timeout=5000)
            check("⑦ 每次開窗從乾淨狀態：搜尋字清空、無 chip 被選中",
            ...
            sets = api(base, "GET", f"/api/workouts?start={time.strftime('%Y-%m-%d')}&end={time.strftime('%Y-%m-%d')}")
            wid = sets[0]["id"]
            saved = api(base, "GET", f"/api/workouts/{wid}")["sets"]
            check("④ 視窗內點動作即進 logger（視窗關閉）、記錄成功；回 picker 後視窗不自己彈開",
            ...
            check("⑧ 既有行為：今日菜單仍在且 4 動作時可捲（F4／F48 不受影響）",
            ...
            page.wait_for_selector(".screen.picker", timeout=8000)
            page.wait_for_timeout(300)
            check("自審①：從視窗內進『動作表現』再返回，視窗接續開著（瀏覽中途離開不該被關掉）",
            ...
            check("自審②：動作庫刷新失敗（離線）仍能開窗並沿用既有清單",
            ...
            page.unroute("**/api/exercises*")
            check("review P2-2：視窗內搜尋失敗時錯誤訊息顯示在視窗內（不被遮罩蓋住）",
                  in_modal_err == 1, f"modal_error_banner={in_modal_err}")
            ...
            check("review P2-3：快速連打時舊回應不覆蓋新結果（清單與搜尋字一致）",
            ...
            page.wait_for_selector(".screen.picker", timeout=8000)
            page.wait_for_timeout(300)
            check("review P2-1：視窗開著時 401 → 重新登入回訓練，視窗不自己彈開",
                  back_to_setup and page.locator(".pick-modal").count() == 0
            ...
            page.wait_for_selector(".home-start", timeout=8000)
            start_free()
            check("② 自由訓練 → 搜尋框／chips／清單／＋自訂動作直接攤開，且無「＋ 臨時加動作」入口鈕",
