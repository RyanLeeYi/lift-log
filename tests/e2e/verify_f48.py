...
def main():
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            check("⑦ APP_VERSION 與 sw.js CACHE_NAME 同步遞增（兩處一致，≥v49）",
                  ver.startswith("v") and int(ver[1:]) >= 49
            ...
            page.wait_for_selector(".home-start", timeout=8000)
            open_templates()
            scrolls = (page.locator(".tpl-rows.scrollable").count() == 1
                       and page.eval_on_selector(".tpl-rows.scrollable", SCROLLS))
            new_out = page.eval_on_selector('button.btn-primary:has-text("＋ 新課表")', OUTSIDE, ".tpl-rows")
            home_out = page.eval_on_selector('.screen.templates button:has-text("← 回首頁")', OUTSIDE, ".tpl-rows")
            check("① 課表 4 份 → 卡片清單固定高度內捲，「＋新課表」「←回首頁」在捲動區外且在畫面內",
                  scrolls and new_out and home_out
            ...
            check("④ 課表列表：滾輪捲動後開/關刪除確認的重繪，捲動位置保留",
            ...
            b_scrolls = (page.locator(".tpl-choice-list.scrollable").count() == 1
                         and page.eval_on_selector(".tpl-choice-list.scrollable", SCROLLS))
            free_out = page.eval_on_selector(".free-choice", OUTSIDE, ".tpl-choice-list")
            check("② 開練挑課表 4 份 → 課表選項捲動，「自由訓練／不用課表」在捲動區外且在畫面內",
                  b_scrolls and free_out and in_viewport(page, ".free-choice")
            ...
            page.wait_for_selector(".screen.picker", timeout=8000)
            c_scrolls = (page.locator(".menu-list.scrollable").count() == 1
                         and page.eval_on_selector(".menu-list.scrollable", SCROLLS))
            head_out = page.eval_on_selector('.menu-head:has-text("今日菜單 ·")', OUTSIDE, ".menu-list")
            ...
            check("③ 今日菜單 4 個動作 → 固定高度內捲；標頭與臨時加動作入口在捲動區外且在畫面內",
            ...
            last_name = page.locator(".menu-list .exercise-item").last.locator("span").first.inner_text()
            ...
            check("④ 今日菜單：滾輪捲動後記一組回 picker 的重繪，捲動位置保留",
            ...
            check("⑥ F4 既有行為不變：記錄後今日菜單進度顯示 1/3 且點亮",
                  sub.startswith("1/3")
            ...
            check("④ 換一次訓練 → 新課表菜單從頂端開始（不沿用上一次的捲動位置）",
                  fresh == 0, f"scrollTop={fresh}")
            ...
            check("④ 重新進課表頁 → 清單從頂端開始（不沿用上次瀏覽位置）",
                  tpl_fresh == 0, f"scrollTop={tpl_fresh}")
