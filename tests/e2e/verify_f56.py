...
def main():
    ...
    try:
        ...
        DAYS_AGO = ([900, 700, 500, 400, 200, 120, 80, 60, 45, 40, 30, 25, 20, 16]
                    + list(range(12, 0, -1)))
        for i in DAYS_AGO:
        ...
        with sync_playwright() as pw:
            ...
            check("⑨ APP_VERSION 與 sw.js CACHE_NAME 同步遞增（兩處一致，≥v57）",
                  ver.startswith("v") and int(ver[1:]) >= 57
            ...
            check("①② 「＋ 記錄」在畫面下方（清單之下、「← 回首頁」之上），版面順序正確",
                  order["open_idx"] == order["home_idx"] - 1
                  and order["list_idx"] < order["open_idx"]
                  and order["open_above_home"] and order["open_below_list"]
            ...
            check("(1)(8) 區間 chips（1M/3M/6M/9M/1Y/2Y/3Y＋自訂）存在、預設 3M",
            ...
            check("(3) 切區間帶 start/end 查後端，圖表與清單同步（1M < 3M <= 1Y）",
            ...
            check("(2) 自訂區間：起>訖 被擋；套用後 chip 選中且清單只含區間內日期",
                  blocked and custom_on == "自訂" and rows_custom > 0
            ...
            check("(4) 切換失敗（5xx）原子性：區間與資料都不動、錯誤有顯示（無半套狀態）",
            ...
            check("(5) 與體重／體脂 toggle 正交（切 metric 不改區間、切區間不改 metric）",
            ...
            check("review P2-3：區間 chips 在 320/360 窄螢幕仍是一行（門檻算式的 30px 前提成立）",
            ...
            check("review P2-2：自訂面板展開（680 高）無溢出、無重疊（允許整頁捲動是明確例外）",
                  panel_geo["panel"] == 1 and panel_geo["overflow"] <= 1 and panel_geo["over_btn"] <= 1,
                  f"{panel_geo}")
            page.locator('.body-range button:has-text("自訂")').click()
            ...
            parts = (page.locator(".body-log-modal .bm-date-picker").count() == 1
            ...
            page.wait_for_timeout(300)
            check("② 視窗含日期／體重／體脂／✓ 記錄／取消；點遮罩空白處關閉",
                  parts and page.locator(".body-log-modal").count() == 0, f"parts={parts}")
            ...
            check("⑥ 開窗預設：日期＝今天、體重帶最新值當起點",
                  d0 == today_iso and w0 not in ("", None), f"date={d0!r} weight={w0!r}")
            ...
            check("④ 送出失敗：視窗不關、輸入保留、錯誤訊息在視窗內（不被遮罩蓋住）",
            ...
            check("③④ 成功記錄（含 F11 補記過去日期）→ 自動關窗、資料落該日、toggle 狀態保留、flash 出現",
                  closed and len(saved) == 1 and saved[0]["weight_kg"] == 95.5
                  and metric_kept == "體脂" and flash == 1,
            ...
            check("③ 換日期帶入該日既有紀錄（同日覆蓋看得見）",
                  w_back == "95.5" and f_back == "22.2", f"weight={w_back!r} fat={f_back!r}")
            page.locator('.body-log-modal button:has-text("取消")').click()
            page.wait_for_timeout(300)
            ...
            # 不納入遞減比較——但各高度都必須不破圖
            check("⑦ 門檻（由 app.css 讀出）成立：≥T+1 填滿且遞減、各高度不破圖",
            ...
            check("review P2-1：flash 出現時 557–612 不再有死帶（按鈕在畫面內或頁面確實可捲、清單不溢出）",
                  flash_on and all((v["btn_in_view"] or v["doc_scroll"] > 0) and v["overflow"] <= 1
            ...
            check("review P2-2：點遮罩關窗會清掉錯誤（不留過期紅字撐高版面）",
                  err_while_open >= 1 and err_after_close == 0
            ...
            page.wait_for_timeout(350)
            edit_ok = (page.locator(".bm-row.editing .bm-edit-weight").count() == 1
            ...
            page.mouse.move(rows_box["x"] + rows_box["width"] / 2, rows_box["y"] + rows_box["height"] / 2)
            ...
            check("⑦ 既有行為：清單編輯仍整筆（F53 ③）、分頁籤捲動記憶仍在（F53 P2-2）",
