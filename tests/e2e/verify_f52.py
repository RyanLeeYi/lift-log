...
def main():
    ...
    try:
        ...
        api(base, "POST", "/api/templates", {"name": "F52 課表", "exercises": [
        ...
        with sync_playwright() as pw:
            ...
            check("⑤ APP_VERSION 與 sw.js CACHE_NAME 同步遞增（兩處一致，≥v53）",
                  ver.startswith("v") and int(ver[1:]) >= 53
            ...
            edit_h = page.eval_on_selector(".screen.template-edit", "e => getComputedStyle(e).height")

            # ② 細捲軸樣式（與其他清單同一組共用規則）
            style = page.eval_on_selector(".tpl-items.scrollable", """e => {
              const s = getComputedStyle(e);
              return { sw: s.scrollbarWidth, sc: s.scrollbarColor, oy: s.overflowY,
                       touch: s.webkitOverflowScrolling || "" };
            }""")
            ref = page.eval_on_selector(".tpl-items.scrollable", "e => 1")
            check("② 編輯頁清單套到細捲軸共用樣式（thin ＋ scrollbar-color ＋ overflow-y: auto）",
            ...
            check("① 加動作視窗：搜尋篩選結果變少時，視窗 top／高度與搜尋框位置皆不位移",
                  n_after < n_before
            ...
            for w, h in ((320, 420), (844, 390)):
                ...
                geo = page.eval_on_selector(".tpl-add-modal", """e => {
                ...
                short[f"{w}x{h}"] = geo
            check("review P2-1：320×420／橫放 844×390 無雙層捲動，「確定加入」完整在視窗內",
            ...
            check("④ 既有行為：部位 chips 在、單選＋確定加入仍正常（F21／F22）",
                  chips_n >= 2 and page.locator(".tpl-item").count() == items_before + 1
            ...
            page.wait_for_timeout(300)
            short_h = page.eval_on_selector(".screen.template-edit", "e => getComputedStyle(e).height")
            page.set_viewport_size({"width": 390, "height": 667})
            page.wait_for_timeout(300)
            ...
            check("③ 四個畫面皆掛 .fills marker；667 高時釘視窗高、390 高時回退成內容高度",
