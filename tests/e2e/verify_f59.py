...
"""
import datetime, json, os, socket, subprocess, sys, tempfile, time, urllib.request, uuid
...
def log_set(base, ex_id, days_ago, weight):
    ...
    api(base, "POST", f"/api/workouts/{w['id']}/sets", {
        "client_uuid": f"f59-{uuid.uuid4().hex[:12]}",
        "exercise_id": ex_id, "set_number": 1, "weight_kg": weight, "reps": 5, "rpe": 7,
    })
...
def main():
    ...
    try:
        ...
        hist_a = api(base, "GET",
                     f"/api/exercises/{ex_a['id']}/history?from={recent}&to={today:%Y-%m-%d}")
        hist_b = api(base, "GET",
                     f"/api/exercises/{ex_b['id']}/history?from={recent}&to={today:%Y-%m-%d}")
        empty_ex = [e for e in exs if e["id"] not in (ex_a["id"], ex_b["id"])][0]
        hist_empty = api(base, "GET",
                         f"/api/exercises/{empty_ex['id']}/history?from={recent}&to={today:%Y-%m-%d}")
        want_a = (today - datetime.timedelta(days=100)).strftime("%Y-%m-%d")
        check("① history 回 first_session_date（全期最早訓練日，查詢區間只框最近 7 天仍回 100 天前）",
        ...
        with sync_playwright() as pw:
            ...
            check("⑦ APP_VERSION 與 sw.js CACHE_NAME 同步遞增（兩處一致，≥v60）",
                  ver.startswith("v") and int(ver[1:]) >= 60
            ...
            st = chip_state(page)
            check("② 動作 A（最早 100 天前）：1M/3M/6M 可用、9M 以上灰、自訂不受限",
            ...
            page.wait_for_timeout(500)
            note_txt = page.locator(".range-note").inner_text() if page.locator(".range-note").count() else ""
            on_after = [k for k, v in chip_state(page).items() if v["on"]]
            check("③ 點停用檔位：顯示說明（含最早訓練日）、選取的區間不變",
            ...
            check("⑤ 切 metric 後說明就地消失（F36 的就地更新不變）",
            ...
            check("⑥ 既有行為：切可用檔位正常（chip 選中、圖表與 PR 卡都在）",
            ...
            check("④ 動作 B（只有今天一次訓練）：初次進頁面就載 1M，且沒有「被選中的那顆是灰的」",
            ...
            check("review P2-2：初次退檔改為純前端過濾（只發 1 次 history 請求），且 1M 內容正確",
